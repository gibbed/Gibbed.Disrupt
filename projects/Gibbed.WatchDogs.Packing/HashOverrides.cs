...
namespace Gibbed.WatchDogs.Packing
{
    ...
    {
        private static readonly Dictionary<ulong, uint> _HashOverrides;

        public static bool TryGet(ulong hash, out uint hashOverride)
        {
            return _HashOverrides.TryGetValue(hash, out hashOverride);
        }

        static HashOverrides()
        {
            // TODO: move this to project config?
            var knownHashOverrides = new Dictionary<uint, string>()
            {
            ...
            var unknownHashOverrides = new Dictionary<ulong, uint>()
            {
            ...
            _HashOverrides = new Dictionary<ulong, uint>();
            foreach (var kv in knownHashOverrides)
            {
                _HashOverrides.Add(
                    Disrupt.FileFormats.Hashing.FNV1a64.Compute(kv.Value.ToLowerInvariant()),
                    kv.Key);
            }
            foreach (var kv in unknownHashOverrides)
            {
                _HashOverrides.Add(kv.Key, kv.Value);
            }
        }
    }
}
